...
void des_db_add_designer(des_db_t * self, designer_t * des)
{
    int rc  = 0;
    sqlite3_stmt * stmt;
    char * sql = "INSERT INTO "TABLE_NAME" (Name, Surname, AverageVertices, Experience, Salary, ProjectCount, HireDate) "
                 "VALUES ( ? ,  ? ,  ? ,  ? ,  ?, ?, ? );";
    if(checkRC(self, rc))
        goto fin;
    ...
    if(checkRC(self, rc))
        goto fin;
    rc = sqlite3_bind_double(stmt, 5, des->salary);
    if(checkRC(self, rc))
        goto fin;
    rc = sqlite3_bind_int(stmt, 6, des->projectsCount);
    if(checkRC(self, rc))
        goto fin;
    rc = sqlite3_bind_int(stmt, 7, (int)(des->hireDate));
    if(checkRC(self, rc))
        goto fin;
    rc = sqlite3_step(stmt);
    if(checkRC(self, rc))
...
designer_t * des_db_get_designers(des_db_t * self, size_t * pcount)
{
	int rc = 0;
	int count = 0;
	int proccessed_count = 0;
