...
void html_builder_designer(http_server_t * self, designer_t * des, char * buffer)
{
	...
	char designer_buf[BUFFER_LENGTH];
	//10
	struct tm  * date = localtime(&(des->hireDate));
	sprintf(designer_buf, "<tr><td>%s</td><td>%s</td><td>%i</td><td>%f</td><td>%i-%i-%i</td>"
		"<td> <a class=\"btn btn-primary btn-default btn-block\" onClick=\"doDelete(\'%i\')\" href=\"\\designers\">Delete</a></td></tr>", 
		des->name, des->surname, des->experience, des->averageVertices, date->tm_year + 1900, date->tm_mon + 1, date->tm_mday, des->id);
	strcat(designers_rows, designer_buf);
	sprintf(designers_table, "<div class=\"container\"><div class=\"jumbotron\"><table class=\"table table-reflow table-bordered table-striped\">"
		"<thead><tr><th>Name</th><th>Surname</th><th>experienceYears</th><th>AverageVertices</th><th>HireDate</th><th></th></tr><tbody>%s</tbody></table></div></div>", designers_rows);
