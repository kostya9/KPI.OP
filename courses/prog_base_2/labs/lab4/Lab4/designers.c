...
int designers_delete_designer(designer designers[], size_t des_count, int id)
{
...
static void designer_to_xml_buffer(xmlTextWriterPtr writer, designer * designer)
{
	int bytes_written;
...
void designer_to_xml_string(char * buffer, size_t size, designer * designer)
{
...
void designer_array_to_xml_string(char * buffer, size_t size, designer designers[], size_t designers_count)
{
	xmlTextWriterPtr writer;
	xmlBufferPtr xml_buf;
	xml_buf = xmlBufferCreate();
...
designer * designer_get_designer_by_id(int id, designer designers[], size_t designers_count)
{
	for (unsigned int i = 0; i < designers_count; i++)
	{
		if (designers[i].id == id)
...
int findUniqueId(designer designers[], size_t count)
{
	int isUnique = 0;
	int id = 0;
	while (!isUnique)
