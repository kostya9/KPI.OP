...
void print_designer(designer_t * des);
...
int main(void)
{
    ...
    size_t count = 0;
    designer_t * designers = des_db_get_filtered_designers(db, 5, 15000.0F, &count);
    printf("\nThere are %i of them\n", count);
    if(count > 0)
    {
        puts("Printing them ...");
...
void one_record_test(des_db_t * db)
{
    ...
    designer_t des1 = {.id = 3, .name = "Peter", .surname = "Bayleish", .averageVertices = 3.14, .experience = 10, .hireDate = time, .salary = 1005.00, .projectsCount = 100};
    puts("\nAdding a record ... ");
    ...
    puts("\nGetting the record ... ");
    designer_t * des1_get = des_db_get_designer(db, 3);
    ...
    puts("\nGetting the record ... ");
    designer_t * des2 = des_db_get_designer(db, 3);
    print_designer(des2);
    free(des2);
