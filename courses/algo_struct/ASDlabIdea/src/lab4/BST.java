...
public class BST {
    ...
    private Student remove(TreeNode toDel, Direction d) {
        // TODO detect case for deleting (no child, one child, two child)
        // cut out node from a tree
        Student removedStudent = toDel.st;
        ...
        if(toDel.left == null){
            ...
            TreeNode successor = findSuccessor(toDel);
            toDel.st = successor.st;
            toDel.id = successor.id;
            replaceToDelWithSuccessor(toDel, successor);
        }
        return removedStudent;

    }

    private void replaceToDelWithSuccessor(TreeNode toDel, TreeNode successor) {
        if(successor.right != null)
        {
            if(successor == toDel.right){
                removeNodeRightChild(successor, Direction.RIGHT);
            }
            else{
                removeNodeRightChild(successor, Direction.LEFT);
            }
        }
        else{

            if(successor == toDel.right){
                removeNodeNoChildren(successor, Direction.RIGHT);
            }
            else{
                removeNodeNoChildren(successor, Direction.LEFT);
            }
        }
    }

    private TreeNode findSuccessor(TreeNode toDel)
    {
